...
**부하테스트로 찾고 고친 것 (k6, 동시 100요청):**

| 지표 | EXCLUDE만 | **비관적 락 전환 후** |
|------|-----------|----------------------|
| 정합성(중복 예약) | 0건 | 0건 |
| 응답 결과 | 1 성공 + 99 **실패(500)** | 1 성공 + 99 **정상 거부(409)** |
| 응답시간(중앙값) | **30초** | **690ms** |
| 총 처리시간 | 39초 | **0.9초** |
| 데드락 | **77회** | **0회** |

EXCLUDE 제약만으로도 정합성은 지켜졌지만, 동시 요청이 GiST 인덱스 락을 제각각 순서로 잡아 **데드락 + 커넥션풀 고갈(30초 타임아웃)** 이 발생했습니다.
운영 경로를 **방(Room) 행을 먼저 잠그는 비관적 락**으로 바꿔 락 순서를 단일화하니 데드락이 사라지고 처리 시간이 39초 → 0.9초로 개선됐습니다. (EXCLUDE 제약은 정합성 백스톱으로 유지)
