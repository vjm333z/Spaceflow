...
export default function BookingPage() {
  ...
  const selectCls =
    'w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-slate-900 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100'
      ...
      {reserve.isSuccess ? (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center">
          ...
          <p className="mt-1 text-sm text-emerald-700">
            {date} {startHour}:00~{startHour + duration}:00
          </p>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="grid grid-cols-3 gap-3">
              ...
              <label className="text-sm">
                <span className="mb-1.5 block font-medium text-slate-600">시작</span>
              ...
              <label className="text-sm">
                ...
                <select value={duration} onChange={(e) => setDuration(Number(e.target.value))} className={selectCls}>
                  {DURATIONS.map((d) => (
                    <option key={d} value={d}>{d}시간</option>
                  ))}
                </select>
              </label>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="grid grid-cols-2 gap-3">
              <label className="text-sm">
                ...
                <input value={guestName} onChange={(e) => setGuestName(e.target.value)} placeholder="홍길동" className={selectCls} />
              </label>
              <label className="text-sm">
                <span className="mb-1.5 block font-medium text-slate-600">연락처 <span className="text-slate-300">(선택)</span></span>
                <input value={guestPhone} onChange={(e) => setGuestPhone(e.target.value)} placeholder="010-0000-0000" className={selectCls} />
              </label>
            </div>
          </div>

          {conflictMessage && (
            <p className="rounded-xl bg-rose-50 px-4 py-3 text-sm text-rose-700">{conflictMessage}</p>
          )}
          ...
          >
            {reserve.isPending ? '예약 중…' : quote ? `${quote.total.toLocaleString()}원 예약하기` : '예약하기'}
          </button>
        </div>
      )}
    </div>
